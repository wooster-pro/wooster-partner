...
?>
        <div class="wrap">
            <h1><?= __('Partenariat Wooster', 'wooster-badge') ?></h1>
            <nav class="nav-tab-wrapper">
                <a href="<?= admin_url() ?>admin.php?page=wooster-partenariat" class="nav-tab nav-tab-active"><?= __('Authentifcation', 'wooster-badge') ?></a>
                <?php if (get_option('partner_license') && get_option('partner_license') != '') { ?>
                    <a href="<?= admin_url() ?>admin.php?page=wooster-badges" class="nav-tab"><?= __('Badges', 'wooster-badge') ?></a>
                    <a href="<?= admin_url() ?>admin.php?page=wooster-visuels" class="nav-tab"><?= __('Visuels', 'wooster-badge') ?></a>
                    <a href="#TB_inline?width=600&amp;height=600&amp;inlineId=modal-chatbot" class="thickbox button" style="float: right"><?= __('Demander un service', 'wooster-badge') ?></a>
                <?php } ?>
            </nav>
    ...
    ?>
        <div class="wrap">
            <h1><?= __('Partenariat Wooster', 'wooster-badge') ?></h1>
            <nav class="nav-tab-wrapper">
                <a href="<?= admin_url() ?>admin.php?page=wooster-partenariat" class="nav-tab"><?= __('Authentifcation', 'wooster-badge') ?></a>
                <a href="<?= admin_url() ?>admin.php?page=wooster-badges" class="nav-tab nav-tab-active"><?= __('Badges', 'wooster-badge') ?></a>
                <a href="<?= admin_url() ?>admin.php?page=wooster-visuels" class="nav-tab"><?= __('Visuels', 'wooster-badge') ?></a>
                <a href="#TB_inline?width=600&amp;height=600&amp;inlineId=modal-chatbot" class="thickbox button" style="float: right"><?= __('Demander un service', 'wooster-badge') ?></a>
            </nav>

            <div class="tab-content">
                <h2><?= __('Badges', 'wooster-badge') ?></h2>
                <?php
                echo show_badge();
                ?>
            </div>
    ...
    ?>
        <div class="wrap">
            ...
            <nav class="nav-tab-wrapper">
                <a href="<?= admin_url() ?>admin.php?page=wooster-partenariat" class="nav-tab"><?= __('Authentifcation', 'wooster-badge') ?></a>
                <a href="<?= admin_url() ?>admin.php?page=wooster-badges" class="nav-tab"><?= __('Badges', 'wooster-badge') ?></a>
                <a href="<?= admin_url() ?>admin.php?page=wooster-visuels" class="nav-tab nav-tab-active"><?= __('Visuels', 'wooster-badge') ?></a>
                <a href="#TB_inline?width=600&amp;height=600&amp;inlineId=modal-chatbot" class="thickbox button" style="float: right"><?= __('Demander un service', 'wooster-badge') ?></a>
            </nav>

            <div class="tab-content">
                <h2><?= __('Logo Wooster', 'wooster-badge') ?></h2>
                <?php
                echo show_visuels();
                ?>
            </div>
...
<?php
    ...
    public function wooster_setup_sections_authentification_page()
    {
        ...
        add_settings_section('licence_section', __('Authentification du certificat', 'wooster-badge'), array($this, 'wooster_sections_authentification_callback'), 'wooster-partenariat', array());
        // add_settings_section('primary_badge_section', __('Intégration du badge officiel', 'wooster-badge'), array($this, 'section_descriptions_callback'), 'wooster-badges');
        // add_settings_section('secondary_badge_section', __('Intégration des badges secondaires', 'wooster-badge'), array($this, 'section_descriptions_callback'), 'wooster-badges');
    }
    ...
    public function wooster_sections_authentification_callback($arguments)
    {
        switch ($arguments['id']) {
            case 'licence_section':
                echo '<p>' . __('Veulliez saisir votre clé de licence de partenaire pour activer l\'intégration des badges.', 'wooster-badge') . '</p>';
                ...
                //     echo '<p>' . __('Le badge officiel est obligatoire.', 'wooster-badge') . '</p>';
                //     break;
                // case 'secondary_badge_section':
                //     echo '<p>' . __('Les badges suivants sont optionnels.', 'wooster-badge') . '</p>';
                //     break;
        }
    }
    ...
    public function wooster_setup_authentification_fields()
    {
        ...
        $fields = array(
            array(
                'uid' => 'partner_license',
                'label' => __('Licence de partenariat :', 'wooster-badge'),
                'section' => 'licence_section',
                'type' => 'text',
                'options' => false,
                'placeholder' => 'XXXX-XXXX-XXXX-XXXX',
                'helper' => __('Ce champ est requis pour l\'affichage du badge.', 'wooster-badge'),
                'supplemental' => __('Votre licence vous a été transmise par mail. Vous pouvez nous contacter si vous ne la retrouvez pas.', 'wooster-badge'),
                'default' => ''
            )
        );
