...
?>
    <div class="notice error">
        <p><?php _e('Partenariat Wooster : Votre <a href=' . $url . '>clé de licence de partenariat' . '</a> avec Wooster n\'a pas été renseigné.', 'wooster-partner'); ?></p>
    </div>
<?php
}
...
function wooster_badge_light_shortcode()
{
    ...
    if (empty(get_option('partner_license'))) {
        ...
        $image = wooster_partenaire_badge_light($certificate, 'https://www.wooster.fr/', 'https://www.wooster.fr/?partner-certificate='. $certificate);
    }
    return $image;
}
...
function wooster_badge_dark_shortcode()
{
    ...
    if (empty(get_option('partner_license'))) {
        ...
        // Construct the image HTML.
        $image = wooster_partenaire_badge_dark($certificate, 'https://www.wooster.fr/', 'https://www.wooster.fr/?partner-certificate=' . $certificate);
    }
    return $image;
}
...
add_filter('plugin_action_links_wooster-partner/wooster-partner.php', 'wooster_partenariat_settings_link');
...
// function wooster_logo_content($type, $color, $width, $height)
{
    ...
    if (empty(get_option('partner_license'))) {
        ...
        if ($type == 'img') {
            // Get the image URL
            if ($color == 'sombre') {
                $image_url = 'https://cdn-img.wooster.fr/app/uploads/wooster-partner/wooster-2.1_logo-hexa-blanc_1000.png';
            } else {
                $color = 'clair';   // Default color version.
                $image_url = 'https://cdn-img.wooster.fr/app/uploads/wooster-partner/wooster-2.1_logo-hexa-noir_1000.png';
            }
